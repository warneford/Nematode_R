...
df <- Data$AverageCellblot$R
temp <- lm(df$Mean ~ df$SD)
library(ggplot2)
ggplot(df, aes(x=Mean, y=SD)) +
  geom_point() +
  geom_smooth(method=lm, colour="red") +
  geom_text(aes(x = max(df$Mean, na.rm = TRUE)*0.2, y = max(df$SD, na.rm = TRUE), label = paste("R^2 is ", format(summary(temp)$adj.r.squared, digits=4)))) +
  ggtitle("Z normalized (Right Orientation) Mean Pal-1 Blot Values versus Standard Deviation") 
rm( df, temp)
